...
var svg2 = d3.select("#textLabel").append("svg").attr({
    width: bbVis.w,
    height: height + margin.top + margin.bottom - 300
}).append("g").attr({
        transform: "translate(" + (margin.left -30) + "," + (margin.top -30) + ")"
    });
...
var legend_labels = ["World Data", "City Data"]
var ext_color_domain = ["gray", "red"]
  ...
  legend.append("rect")
  .attr("x", 180)
  .attr("y", function(d, i){ return (bbVis.h - (i*ls_h) - 140);})
  .attr("width", ls_w)
  .attr("height", ls_h)
  ...
  legend.append("text")
  .attr("x", 210)
  .attr("y", function(d, i){ return (bbVis.h - (i*ls_h) - 124);})
      ...
      svg.selectAll("circle")
         .data(data_list)
           ...
           .on("mouseover", function(d){tooltip.style("visibility", "visible");
                    tooltip.html("<div id=textLabel></div>") ; make_bars(d);})
